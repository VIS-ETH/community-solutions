...
class Migration(migrations.Migration):
    ...
    CREATE OR REPLACE VIEW scoreboard_userscore (id, user_id, score, upvotes, downvotes, document_likes, answers, legacy, official, cuts, documents, comments) AS
    SELECT row_number() OVER ( ORDER BY (COALESCE(auv.count, 0) - COALESCE(adv.count, 0) + COALESCE(dv.count, 0)) DESC ) AS id,
    au.id AS user_id,
    (COALESCE(auv.count, 0) - COALESCE(adv.count, 0) + COALESCE(dv.count, 0)) AS score,
    ...
    reverse = """
    DROP VIEW IF EXISTS scoreboard_userscore;
    CREATE OR REPLACE VIEW scoreboard_userscore (id, user_id, upvotes, downvotes, document_likes, answers, legacy, official, cuts, documents, comments) AS
        SELECT au.id as id,
        au.id AS user_id,
        COALESCE(auv.count, 0) AS auv_count,
        COALESCE(adv.count, 0) AS adv_count,
