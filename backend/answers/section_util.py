...
def get_answer_response(request, answer: Answer, ignore_exam_admin=False):
    ...
    try:
        ...
        return {
            ...
            "authorId": (
                answer.author.username if answer.kind == Answer.Kind.PERSONAL else ""
            ),
            "authorDisplayName": (
                get_my_user(answer.author).displayname()
...
def get_answersection_response(request, section):
    ...
    return {
        ...
        "allow_new_official_answer": not prepared_query.filter(
            kind=Answer.Kind.OFFICIAL
        ).exists(),
