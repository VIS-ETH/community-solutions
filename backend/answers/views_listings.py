    ...
    res = [
        {
            ...
            "import_claim_displayname": (
                get_my_user(exam.import_claim).displayname()
    ...
    res = [
        {
            ...
            "payment_uploader": (
                exam.oral_transcript_uploader.username
                if exam.oral_transcript_uploader
                else None
            ),
        }
...
def get_by_user(request, username, page=-1):
    sorted_answers = Answer.objects.filter(
        author__username=username, kind=Answer.Kind.PERSONAL
