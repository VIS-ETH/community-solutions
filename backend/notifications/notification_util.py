...
def new_comment_to_answer(answer, new_comment):
    if answer.is_legacy_answer or answer.is_official_answer:
        return
    send_notification(
        new_comment.author,
        answer.author,
...
def _new_answer_to_answer(old_answer, new_answer):
    if old_answer.is_legacy_answer or old_answer.is_official_answer:
        return
    send_notification(
        new_answer.author,
        old_answer.author,
...
def new_answer_to_answer(new_answer):
    for other_answer in Answer.objects.filter(
        answer_section=new_answer.answer_section, is_legacy_answer=False, is_official_answer=False
    ):
